...
saveVendorProduct(formData:any): Observable<any> {
  ...
  GetAllDesignerItemsById(venderid:any) {   
    alert("working");
    let token = TokenService.getToken();
     let headers = new HttpHeaders().set("Authorization", "Bearer " + token);
     return this.http.get<any>(this.apiUrl +"/managedesignerItem/GetallDesignerItemsById/itemid?venderid"+venderid, {headers});
   }


  // get all vender product
  GetallDesignerItems() {
    let token = TokenService.getToken();
     let headers = new HttpHeaders().set("Authorization", "Bearer " + token);
     return this.http.get<any>(this.apiUrl + "/managedesignerItem/GetallDesignerItems", {headers });
   }
